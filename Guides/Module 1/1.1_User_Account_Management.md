...
STEP 2: Create Password Validation Helper
├── Validate OWASP requirements: min 15 chars, upper, lower, digit, special
└── Return list of validation errors if any fail
...
├── RegisterUser(dto) → create new user, generate OWASP-compliant temp password, hash with PasswordHasher, send email
...
### Concept 2: Password Hashing with PBKDF2 (What it is)
...
**How it works:**
```
1. User creates account with password "MyStr0ng!Pass#2024"
2. We hash it using PasswordHasher: hasher.HashPassword(user, "MyStr0ng!Pass#2024") → "AQAAAAIA...XYZ..."
3. We store the HASH in the database, not the actual password
4. When user logs in with "MyStr0ng!Pass#2024":
   - Use PasswordHasher to verify: hasher.VerifyHashedPassword(user, storedHash, "MyStr0ng!Pass#2024")
   - Returns: Success, Failed, or NeedsRehash
   - If Success → login successful
```

**Why PBKDF2 (PasswordHasher<T>):**
- Built into ASP.NET Core (no external packages needed)
- Uses PBKDF2 algorithm with SHA256 (NIST approved)
- Applies 100,000+ iterations (slow by design to prevent brute force)
- Automatically generates unique salt for each password
- OWASP recommended algorithm
- Part of Microsoft.AspNetCore.Identity namespace
...
**Alternatives:**
- **BCrypt**: Good but requires external package
- **Argon2**: Newer but less supported in .NET
...
- Using weak passwords for temporary credentials
...
### Step 3: Create Password Validation Helper

**File: `backend/Services/PasswordValidator.cs`**
...
public class UserService : IUserService
{
    ...
    private string GenerateTempPassword()
    {
        ...
        // Ensure at least one of each required type (OWASP compliance)
...
*(No BCrypt package needed - we use ASP.NET Core's built-in `PasswordHasher<T>` which implements PBKDF2)*
...
1. How does password hashing work with PBKDF2 (PasswordHasher<T>)?
...
|---|-----------|---------|----------|
| 1.1 | Register with all required fields | `{ "employeeNumber": "0001", "email": "john@speedex.com", "firstName": "John", "lastName": "Doe", "role": 0 }` | `201 Created`, check email for temp password |
| 1.2 | Register with optional fields | Add `"middleName": "M"`, `"suffix": "Jr"`, `"contactNumber": "09171234567"`, `"departmentId": "{guid}"`, `"jobPositionId": "{guid}"` | `201 Created`, all fields populated |
| 1.3 | Duplicate employee number | Send same request as 1.1 again | `400`, "Employee number already exists" |
| 1.4 | Duplicate email (different casing) | `{ "employeeNumber": "0002", "email": "JOHN@speedex.com", ... }` | `400`, "Email already exists" |
| 1.5 | Invalid email format | `"email": "notanemail"` | `400` (model validation error) |
| 1.6 | Missing required fields | Omit `firstName` or `email` | `400` (model validation error) |
| 1.7 | Invalid department ID | `"departmentId": "00000000-0000-0000-0000-000000000000"` | `400`, "Department not found or is inactive" |
| 1.8 | Invalid job position ID | `"jobPositionId": "00000000-0000-0000-0000-000000000000"` | `400`, "Job position not found or is inactive" |
| 1.9 | Optional fields as null | Omit `departmentId` and `jobPositionId` entirely | `201 Created`, those fields are `null` |

#### 2. GET `/api/users`

| # | Test Case | Example URL | Expected |
|---|-----------|-------------|----------|
| 2.1 | Get all users | `GET /api/users` | `200`, list sorted by LastName then FirstName |
| 2.2 | Search by name | `GET /api/users?search=john` | Returns matching users (case-insensitive) |
| 2.3 | Search by email | `GET /api/users?search=@speedex.com` | Returns matching users |
| 2.4 | Search by employee number | `GET /api/users?search=0001` | Returns matching users |
| 2.5 | Filter by role | `GET /api/users?role=Manager` | Returns only Managers |
| 2.6 | Filter by department | `GET /api/users?departmentId={guid}` | Returns only users in that department |
| 2.7 | Combine filters | `GET /api/users?search=john&role=Encoder&departmentId={guid}` | Returns users matching all |
| 2.8 | Empty search | `GET /api/users?search=` | Returns all users |
| 2.9 | Invalid role string | `GET /api/users?role=SuperAdmin` | Returns all users (filter ignored) |
| 2.10 | Case-insensitive role | `GET /api/users?role=manager` | Works |

#### 3. GET `/api/users/{id}`

| # | Test Case | Example | Expected |
|---|-----------|---------|----------|
| 3.1 | Valid user ID | `GET /api/users/{valid-guid}` | `200`, full user data |
| 3.2 | Non-existent ID | `GET /api/users/00000000-0000-0000-0000-000000000000` | `404`, "User not found" |
| 3.3 | Invalid GUID format | `GET /api/users/abc` | `404` (route constraint rejects) |
| 3.4 | Deactivated user | `GET /api/users/{deactivated-user-guid}` | `200`, `IsDeactivated = true` |

#### 4. GET `/api/users/employee-number/{employeeNumber}`

| # | Test Case | Example | Expected |
|---|-----------|---------|----------|
| 4.1 | Valid employee number | `GET /api/users/employee-number/0001` | `200`, correct user |
| 4.2 | Non-existent number | `GET /api/users/employee-number/9999` | `404` |
| 4.3 | Without leading zeros | `GET /api/users/employee-number/1` | `404` (only `0001` matches) |

#### 5. PUT `/api/users/{id}`

| # | Test Case | Example | Expected |
|---|-----------|---------|----------|
| 5.1 | Update own profile | `{ "firstName": "Jonathan" }` | `200`, FirstName updated |
| 5.2 | Update email | `{ "email": "newemail@speedex.com" }` | `200`, `IsEmailVerified` reset to `false` |
| 5.3 | Email already taken | `{ "email": "existing@speedex.com" }` | `400`, "Email already exists" |
| 5.4 | Same email different casing | `{ "email": "JOHN@speedex.com" }` | `200`, no change (case-insensitive) |
| 5.5 | Empty body | `{}` | `200`, only `UpdatedAt` changes |
| 5.6 | Invalid email format | `{ "email": "notanemail" }` | `400` (model validation) |
| 5.7 | Update deactivated user | PUT to a deactivated user's ID | `400`, "Cannot update a deactivated user" |
| 5.8 | Non-existent user ID | PUT to `00000000-...` | `404`, "User not found" |

#### 6. PATCH `/api/users/{id}/deactivate`

| # | Test Case | Example | Expected |
|---|-----------|---------|----------|
| 6.1 | Deactivate active user | `PATCH /api/users/{guid}/deactivate` | `200`, `IsDeactivated = true`, `IsActive = false` |
| 6.2 | Deactivate already deactivated | Send same request again | `400`, "User is already deactivated" |
| 6.3 | Non-existent user | `PATCH /api/users/00000000-.../deactivate` | `404` |

#### 7. PATCH `/api/users/{id}/activate`

| # | Test Case | Example | Expected |
|---|-----------|---------|----------|
| 7.1 | Activate deactivated user | `PATCH /api/users/{guid}/activate` | `200`, `IsDeactivated = false`, `IsActive = true` |
| 7.2 | Activate already active user | Send same request again | `400`, "User is already active" |
| 7.3 | Non-existent user | `PATCH /api/users/00000000-.../activate` | `404` |

#### 8. GET `/api/users/next-employee-number`

| # | Test Case | Example | Expected |
|---|-----------|---------|----------|
| 8.1 | With existing users | `GET /api/users/next-employee-number` | Returns next number (e.g., `"0004"` if 3 exist) |
| 8.2 | With no users | Clear users table, then call | Returns `"0001"` |

#### 9. Integration Flows

| # | Test Case | Steps | Expected |
|---|-----------|-------|----------|
| 9.1 | Full lifecycle | Register → Deactivate → Try Update → Reactivate → Update | Update blocked while deactivated, succeeds after reactivation |
| 9.2 | Email uniqueness | Register `john@test.com` → Register `JOHN@test.com` | Second registration blocked |
| 9.3 | Deactivated user in list | Deactivate user → GET `/api/users` | Deactivated user still appears in list |
...
|----------|-------|-------------|
| PasswordValidator | 13 tests | Null, empty, whitespace, too short, missing uppercase/lowercase/digit/special, valid password, exactly 15 chars, 14 chars, multiple failures, unicode |
| Temp Password Generation | 4 tests | Always OWASP-compliant, always 16 chars, always unique (1000 iterations), contains all required character types |
| Employee Number Generation | 5 tests | Empty DB returns "0001", sequential numbering, gap filling, non-numeric values ignored, D4 format |
| MapToResponseDTO | 4 tests | FullName with all fields, FullName with missing optional fields, no double spaces, DepartmentName/JobPositionName mapping |
| Email Case-Insensitive Comparison | 3 tests | Same email different casing detected, different emails not flagged, null/empty handling |

#### xUnit Code

**File: `backend/Tests/PasswordValidatorTests.cs`**
...
**File: `backend/Tests/TempPasswordGenerationTests.cs`**
...
**File: `backend/Tests/EmployeeNumberGenerationTests.cs`**
...
**File: `backend/Tests/MapToResponseDTOTests.cs`**
...
{
    private object MapToFullName(User user)
    {
...
**File: `backend/Tests/EmailCaseInsensitiveTests.cs`**
