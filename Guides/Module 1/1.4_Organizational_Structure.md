...
STEP 7: Create Transfer Service & Controller (for FR-016)
├── TransferService → handles user transfer logic
├── TransferController → POST /api/transfer/{userId}
├── Input: userId, newDepartmentId, newJobPositionId
├── Check if user exists and is active
├── Check if new department and position exist and are active
├── Verify job position belongs to target department
├── Update user's DepartmentId and JobPositionId
└── Return success message

STEP 8: Update Program.cs
├── Register DepartmentService
├── Register JobPositionService
├── Register TransferService
└── Seed default departments on startup
```
...
Create these folders in the `backend` directory:
- `Models/Enums/`
- `Models/DTOs/`

*(We'll add password-related packages later in Sub-Module 1.1)*
...
public class DepartmentService : IDepartmentService
{
    ...
    public async Task<ApiResponseDTO<List<DepartmentResponseDTO>>> GetAllAsync()
    {
        ...
        return ApiResponseDTO<List<DepartmentResponseDTO>>.Success(response);
    }
...
### Step 10.5: Create Transfer Service (for FR-016)

**File: `backend/Services/ITransferService.cs`**
...
**File: `backend/Services/TransferService.cs`**
...
public class TransferService : ITransferService
{
    ...
    public async Task<ApiResponseDTO<bool>> TransferUserAsync(Guid userId, TransferUserDTO dto)
    {
        ...
        // Check if new job position exists and is active
        ...
        // Update user's department and position
...
### Step 10.6: Create TransferController

**File: `backend/Controllers/TransferController.cs`**
...
*(Password hashing packages will be added in Sub-Module 1.1)*
